...
- [ ] 测试入口保护：Basic Auth 配置及独立随机凭据初始化已实现，本地匿名/错误密码拒绝、正确密码访问验证通过；服务器尚待应用，应用内登录和用户权限仍待实现。
...
- 2026-09-17：新增 PR/main/develop 自动部署检查 CI，使用 GitHub 临时 runner 和随机测试数据库密码；新增 5 项 HTTP 集成检查，无付费 AI 调用，无正式服访问。
