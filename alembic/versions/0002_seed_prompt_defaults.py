...
出厂默认 6 套提示词。内容取自定稿时 core/prompts.py 的 DEFAULT_PROMPTS，
内嵌为固定数据——任何新环境 `alembic upgrade head` 即得完整可用库，不依赖仓库外文件。
同时写入 prompt_defaults（出厂基线，只读）与 prompt_templates（当前生效版的初始值）。
本机已改过的提示词随后用 scripts/import_prompts_json.py 一次性覆盖 prompt_templates。
...
_SEED = {'copywriting': {'description': 'Claude 看生成的广告图，输出多套标题文案',
                 'name': '④ 看图写文案',
                 'template': '你是一位 Meta 广告投放的资深中文/英文双语文案。请仔细观察这张广告图片，结合产品信息，为它写 {title_count} '
                             '套可直接投放的广告文案。\n'
                             '\n'
                             '产品信息：\n'
                             '{product_info}\n'
                             ...
                             '\n'
                             '要求：\n'
                             '1. 每套文案包含：headline（标题，英文，40 字符以内，有钩子）和 primary_text（主文案，英文，1-3 '
                             '句，口语化、有场景代入感，可带 1-2 个 emoji，结尾带行动号召）\n'
                             ...
                             '{\n'
                             '  "copies": [\n'
                             '    {"angle": "文案角度（中文标注）", "headline": "英文标题", "primary_text": '
                             '"英文主文案"}\n'
                             '  ]\n'
                             '}',
                 'variables': ['product_info', 'main_scene', 'sub_scene', 'title_count']},
 'image_gen': {'description': '场景挖掘变量+品牌名/广告语言/比例直接填入本模板，渲染后原样发给生图模型（不再经 Claude '
                              '生成提示词）；{reference_style_image} 在发送瞬间按是否带风格图填充',
               'name': '② 生图总提示词（直发生图模型）',
               'template': '你是一名资深 Meta 广告创意总监和商业广告海报生图专家。\n'
                           ...
                           '**不要给出多个方案。**\n'
                           '**直接生成最终广告海报图片。**',
               'variables': ['reference_style_image',
                             'main_scene',
                             'sub_scene',
                             'audience',
                             'trigger',
                             ...
                             'ad_language',
                             'brand_name']},
 'image_refine': {'description': '按用户修改意见在原图基础上编辑图片（直接发给生图模型）',
                  'name': '分支 · 图片修改（对话）',
                  'template': 'Edit this image according to the following instruction, while '
                              'keeping everything else (subject, product, composition, lighting, '
                              'style, colors) unchanged. The instruction may be written in '
                              'Chinese:\n'
                              '\n'
                              '{feedback}\n'
                              '\n'
                              'Absolutely no text, no letters, no watermark, no logo overlays, no '
                              'borders in the image.',
                  'variables': ['feedback']},
 'ratio_adapt': {'description': '双尺寸时把 4:5 母版图改成其他比例（直接发给生图模型，内容保持不变）',
                 'name': '分支 · 尺寸改版',
                 'template': 'Recompose this exact image into a {target_ratio} aspect ratio while '
                             'keeping the content identical: the same subject, the same product, '
                             'the same people, the same background, the same lighting, colors and '
                             'style. Do not add or remove any elements. Only adjust the framing '
                             'and composition so the scene fits the {target_ratio} format '
                             'naturally. Absolutely no text, no letters, no watermark, no logo '
                             'overlays, no borders in the image.',
                 'variables': ['target_ratio']},
 'refine_text': {'description': '各环节文字结果（场景/提示词/文案）按用户修改意见迭代修订',
                 'name': '分支 · 结果修改（对话）',
 ...
 'scene_mining': {'description': '根据产品信息+排除场景列表，挖掘带评分的主/细分场景（目标用户/触发时刻/痛点渴望/产品使用链路 + 五维评分）',
                  'name': '① 场景挖掘',
                  'template': '你是一位资深的 Meta（Facebook / '
                              'Instagram）效果广告策略专家，擅长从产品能力、目标人群、触发时刻和购买动机中，挖掘可以直接用于广告创意测试的高转化场景。\n'
                              ...
                              '\n'
                              '请先在内部生成不少于 30 '
                              '个候选细分场景，完成去重、分类和评分后，把去重后的候选**全部输出**——不做分数门槛淘汰，评分供使用者自行筛选。不要输出分析过程。\n'
                              '\n'
                              '## 一、主场景定义\n'
                              '\n'
...
_INSERT_SQL = (
    "INSERT INTO {table} (key, name, description, variables, template) "
    "VALUES (:key, :name, :description, CAST(:variables AS jsonb), :template)"
)


def upgrade() -> None:
    bind = op.get_bind()
    for table in ("prompt_defaults", "prompt_templates"):
        stmt = sa.text(_INSERT_SQL.format(table=table))
        for key, item in _SEED.items():
            bind.execute(stmt, {
                "key": key,
                "name": item["name"],
...
def downgrade() -> None:
    bind = op.get_bind()
    for table in ("prompt_templates", "prompt_defaults"):
        for key in _SEED:
            bind.execute(sa.text(f"DELETE FROM {table} WHERE key = :key"), {"key": key})
